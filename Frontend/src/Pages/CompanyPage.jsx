...
import { faEnvelope, faPhone, faEdit, faUser, faTimes, faEllipsisV, faCalendarAlt, faClock, faMapMarkerAlt,faPlus } from '@fortawesome/free-solid-svg-icons';
...
const events = [
  {
    ...
    type: "In-person",
    transparency: "public",
    imageUrl: "https://img.freepik.com/free-vector/gradient-car-rental-design-template_23-2149264700.jpg",
  ...
  },
  {
    ...
    type: "Hybrid",
    transparency: "public",
    imageUrl: "https://img.freepik.com/free-vector/artificial-intelligence-concept-facebook-post_23-2150394426.jpg",
  ...
  },
  {
    ...
    imageUrl: "https://img.freepik.com/free-vector/gradient-texture-technology-webinar_23-2149094510.jpg",
    createdDate: "Oct 5, 2024",
  ...
  },
  {
    ...
    type: "Hybrid",
    transparency: "private",
    imageUrl: "https://img.freepik.com/free-vector/webinar-template-photographer-career-hobby_23-2150302583.jpg",
...
const CompanyPage = () => {
  ...
  const [user, setUser] = useState({
    image: localStorage.getItem('userImage') || 'https://via.placeholder.com/150',
    username: 'Shreya',
  ...
  ? checkSlotConfirmation(selectedEvent.createdDate, selectedEvent.slotConfirmedDate) 
  : false; 
    ...
    <>
      ...
      <div className="flex">
        ...
        <div className="fixed top-[4.5rem] left-4 z-50  lg:hidden">
          <button onClick={toggleMenu} className="p-2 h-8 w-8 bg-indigo-200 flex justify-center items-center rounded-full text-red-500  ">
            <FontAwesomeIcon icon={menuVisible ? faTimes : faEllipsisV} style={{ fontSize: '1.3rem'}} className="font-bold"  />
          </button>
        </div>

        <div
          className={`fixed lg:z-10 z-40 top-16 left-0 bg-[#081647] text-white rounded-tr-2xl rounded-br-2xl shadow-2xl p-4 flex flex-col items-center transition-transform duration-300 transform ${menuVisible ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0`}
        ...
        >
          ...
          <div className="mt-auto w-[100%] flex flex-col text-xs items-center">
            <div className="w-32 w-[95%] border-b-2 border-gray-200 m-2 rounded-2xl mt-10 mb-4"></div>
            &copy;Eventek2024.
          </div>
        </div>
        ...
        {/* EVENT DETAILS */}
        <div
          className={` ${menuVisible ? 'blur-sm lg:blur-none' : ''} mt-8 mb-8 w-full  lg:w-5/6 ml-8 lg:ml-[14rem] mr-8 lgoverflow-y-auto space-y-4`}
          style={{ height: 'calc(100vh - 60px)' }}
        >
        <div className="mt-4 flex flex-row justify-between items-center">
            <h2 className="text-gradient2 text-3xl xds:text-3xl sm:text-5xl font-bold w-[60%]  " style={{ fontFamily: '"quick"' }}>
              Events &nbsp;Ground
            </h2>
            <button
            ...
            >
                <FontAwesomeIcon icon={faPlus} className="text-white mr-2 font-bold" />Create Event
            </button>
        </div>
          ...
          {events.map((event, index) => (
            ...
            >
              ...
              <div className="flex flex-col justify-center items-center w-full">
                    <h3 className="text-lg text-gradient1 xds:text-2xl mb-4 text-indigo-800 font-bold font-serif">{event.name}</h3>
                    <div className="flex justify-center items-center space-x-2 xds:space-x-8 sm:space-x-12 md:space-x-6 lg:space-x-8">
                          ...
                          <div className="flex items-center text-xs xds:text-md sm:text-lg md:text-sm lg:text-sm font-bold text-white">
                            <FontAwesomeIcon icon={faCalendarAlt} className="mr-2 text-indigo-800" />
                            <span>{event.date}</span>
                          </div>

                          {/* Event Time */}
                          <div className="flex items-center text-xs xds:text-md sm:text-lg md:text-xs lg:text-sm font-bold text-white">
                            <FontAwesomeIcon icon={faClock} className="mr-2 text-indigo-800" />
                    ...
                    <button
                      ...
                      onClick={() => openModal(event)} // Pass the event here
                    >
                      Track Event
                    </button>
              </div>
      ...
      {isModalVisible && selectedEvent && (
        ...
        >
          <div className="bg-white p-6 rounded-lg shadow-lg w-[90%] sm:w-[50%]">
            <h2 className="text-gradient1 text-4xl font-bold text-center mb-6" style={{ fontFamily: '"quick"' }}>
              Event Tracker
            </h2>
            <div className="relative">
              <div className="relative flex flex-col">
                        ...
                        <div className="flex items-center">
                          <div className={`w-4 h-4 rounded-full ${selectedEvent.createdDate ? 'bg-blue-600' : 'bg-gray-400'} mr-2`}></div>
                          <span className="text-sm xds:text-lg font-bold">Event Creation</span>
                        </div>
                        <p className="text-green-500 text-sm ml-6">Event created successfully</p>
                        ...
                        {/* Slot Confirmation */}
                        <div className="flex items-center">
                          <div className={`w-4 h-4 rounded-full ${selectedEvent.slotConfirmedDate ? (isSlotConfirmed ? 'bg-blue-600' : 'bg-gray-400') : 'bg-gray-400'} mr-2`}></div>
                          <span className="text-xs xds:text-lg font-bold">Slot Confirmation (Within 24 hours)</span>
                        </div>
                        {!selectedEvent.slotConfirmedDate ? (
                          <p className="text-yellow-500 text-sm ml-6">Slot not yet decided</p>
                        ) : !isSlotConfirmed && selectedEvent.createdDate ? (
                          <p className="text-red-500 text-sm ml-6">
                            Sorry! We failed to schedule a slot for you, and further processing is not possible.
                          </p>
                        ) : (
                        ...
                        <div className={`w-1 ${checkEventCompletion(selectedEvent.date, selectedEvent.slotConfirmedDate, selectedEvent.createdDate) ? 'bg-blue-600' : 'bg-gray-400'} h-16`} style={{ marginLeft: '0.35rem', marginRight: '1.5rem', marginTop: '-2.8rem' }}></div>



                        {/* Event Done */}
                        <div className="flex items-center">
                          <div className={`w-4 h-4 rounded-full ${checkEventCompletion(selectedEvent.date, selectedEvent.slotConfirmedDate, selectedEvent.createdDate) ? 'bg-blue-600' : 'bg-gray-400'} mr-2`}></div>
                          <span className="text-xs xds:text-lg font-bold">Event Completion</span>
                        </div>
                        {checkEventCompletion(selectedEvent.date, selectedEvent.slotConfirmedDate, selectedEvent.createdDate) ? (
                          <p className="text-green-500 text-sm ml-6">Event completed successfully</p>
                        ) : (
                          <p className="text-red-500 text-sm ml-6">Event is not yet complete</p>
                        )}
                      </div>
                    </div>
